...
from ewancli.agent import Agent
from ewancli.config import EwanCliConfig, LlmConfig, load_config
from ewancli.llm import calculate_cost, create_llm_client, get_builtin_price_profile
from ewancli.llm.openai_compatible import OpenAICompatibleClient
from ewancli.tools import ToolRegistry
from ewancli.types import Message, Usage
...
def test_agent_propagates_connection_error_without_raising(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("ewancli.agent.agent.SnapshotService", NoopSnapshotService)
    monkeypatch.setattr(httpx.AsyncClient, "stream", fail_stream)
...
def test_unknown_model_context_and_prices_can_be_configured(tmp_path) -> None:
    project_config = tmp_path / ".ewancli" / "config.json"
