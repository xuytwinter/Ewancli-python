...
from ewancli.skill import SkillContextBuffer, SkillMatcher, SkillRegistry, SkillStateStore
from ewancli.tools import ToolRegistry, get_builtin_tools
from ewancli.tools.base import ToolContext
from ewancli.tools.builtins import load_skill
...
def test_skill_registry_layers_and_state(tmp_path, monkeypatch):
    ...
    _write_skill(project / ".ewancli" / "skills", "project-only", "project desc", "v2")
...
def test_load_skill_pushes_body_into_context_buffer(tmp_path, monkeypatch):
    monkeypatch.setenv("HOME", str(tmp_path / "home"))
    _write_skill(tmp_path / ".ewancli" / "skills", "demo", "demo desc", "v1", body="demo body")
...
def test_skill_registry_create_update_and_scope(tmp_path, monkeypatch):
    ...
    assert created.path == project / ".ewancli" / "skills" / "release-check" / "SKILL.md"
...
def test_save_skill_tool_requires_approval_and_persists_after_approval(tmp_path, monkeypatch):
    ...
    assert not (tmp_path / ".ewancli" / "skills" / "test-workflow" / "SKILL.md").exists()
    ...
    assert (tmp_path / ".ewancli" / "skills" / "test-workflow" / "SKILL.md").is_file()
