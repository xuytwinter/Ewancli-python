...
from ewancli.entrypoints.model_selector import ModelSelectorState
from ewancli.entrypoints.repl import _activate_model
...
def test_activate_model_rebuilds_live_client_without_restart(tmp_path, monkeypatch):
    ...
    assert "You are Ewancli" in agent.system_prompt
...
def test_glm_startup_uses_official_zai_api_key_and_context(tmp_path, monkeypatch):
    ...
    config = load_config(
        ...
        env={
            "EWANCLI_PROVIDER": "glm",
            "EWANCLI_MODEL": "glm-5.2",
