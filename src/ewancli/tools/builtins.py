...
from ewancli.policy import CommandGuard
from ewancli.rag import CodeIndex
from ewancli.skill import SkillRegistry
from ewancli.snapshot import SnapshotService
from ewancli.tools import file_ops as fops
from ewancli.tools.base import Tool, ToolContext, ToolResult, object_schema
from ewancli.tools.file_ops import FileOpResult
from ewancli.web import fetch_url, search_web
...
def get_builtin_tools() -> list[Tool]:
    tools = [
        ...
        Tool(
            name="load_skill",
            description="Load a named Ewancli skill manual from user/project skill directories.",
            parameters=object_schema(
        ...
        Tool(
            name="save_skill",
            description=(
                "Persist a reusable Ewancli workflow as a project or user skill "
                "after user approval."
            ),
            parameters=object_schema(
                {
        ...
        Tool(
            name="revert_turn",
            description="Restore the workspace to a previous Ewancli side-history snapshot.",
            parameters=object_schema(
