"""cli.py — Ewancli terminal AI agent CLI entry point.
...
Usage modes::
    ...
    ewancli
    ewancli --api-key sk-xxx
    ewancli --model deepseek-chat --provider deepseek

    # Single-shot prompt
    ewancli -p "list all Python files"
    ewancli -p "refactor this module" --mode plan

    # Tooling
    ewancli doctor              # system health check
    ewancli serve               # Runtime HTTP API
    ewancli mcp serve           # Expose tools via MCP
    ewancli mcp init-chrome     # Configure Chrome DevTools MCP
"""
...
from ewancli.config import get_config_paths, load_config
from ewancli.entrypoints.repl import start_repl
from ewancli.llm import create_llm_client
from ewancli.mcp import load_mcp_server_specs, serve_http, serve_stdio, write_chrome_devtools_config
from ewancli.runtime import RuntimeApiServer
from ewancli.runtime.api import runtime_api_key

app = typer.Typer(
    name="ewancli",
    help="Ewancli — Terminal AI Agent in Python (Claude Code alternative)",
...
def _version_callback(value: bool) -> None:
    if value:
        typer.echo(f"ewancli {__version__}")
        raise typer.Exit()
...
def main(
    ...
    """Ewancli — a terminal AI agent that works in your workspace.
...
def runtime_serve(
    ...
    api_key: Annotated[
        str | None,
        typer.Option("--api-key", help="Runtime API key. Defaults to EWANCLI_RUNTIME_API_KEY."),
    ] = None,
    cwd: Annotated[Path | None, typer.Option("--cwd", help="Working directory")] = None,
) -> None:
...
def mcp_serve(
    ...
    """Expose Ewancli tools via the Model Context Protocol."""
    ...
    *,
    ...
    if not config.llm.api_key:
        typer.echo(
            ...
            "via the EWANCLI_API_KEY environment variable, or in "
            "~/.ewancli/config.json or .ewancli/config.json.",
            err=True,
        )
        raise typer.Exit(1)
