...
from ewancli import __version__
from ewancli.agent import Agent, AgentOrchestrator, PlanExecuteAgent
from ewancli.bootstrap import build_tool_registry
from ewancli.config import EwanCliConfig, config_to_public_dict
from ewancli.entrypoints.model_selector import ModelSelectorState, run_model_selector
from ewancli.llm import create_llm_client
from ewancli.llm.model_profiles import (
    DEFAULT_MODEL_PROFILES,
    PROVIDER_DEFAULTS,
    CustomModelStore,
    ModelProfile,
)
from ewancli.memory import MemoryManager
from ewancli.policy import AuditLog
from ewancli.prompt import PromptAssembler
from ewancli.rag import CodeIndex
from ewancli.render import RichRenderer
from ewancli.runtime import DurableTaskManager
from ewancli.skill import SkillRegistry
from ewancli.snapshot import SnapshotService
from ewancli.tools import ToolRegistry
...
async def start_repl(cwd: str, config: EwanCliConfig) -> None:
    console = Console()
    ...
    history_path = Path.home() / ".ewancli" / "history" / "prompt_history.txt"
...
async def _handle_slash(
    ...
    if command == "/help":
        ...
        table = Table(title="Ewancli Context")
        ...
        console.print("Use `ewancli mcp serve --transport stdio|http --port 3000` to expose tools.")
    elif command == "/task":
...
async def _memory_command(arg: str, console: Console, cwd: str, config: EwanCliConfig) -> None:
...
def config_base_url(provider: str) -> str:
    from ewancli.llm.factory import DEEPSEEK_BASE_URL, OPENAI_BASE_URL, PROVIDER_BASE_URLS
...
def _provider_api_key_env(provider: str) -> str:
    return {
        ...
        "openai-compatible": "EWANCLI_API_KEY",
    }.get(provider.lower(), "EWANCLI_API_KEY")
...
def _task_command(arg: str, console: Console, cwd: str) -> None:
    manager = DurableTaskManager(Path.home() / ".ewancli" / "tasks" / "tasks.db", scope=cwd)
    sub, _, rest = arg.partition(" ")
    if sub == "add" and rest:
        try:
        ...
        console.print(
            f"Queued {task_id} ({mode}). Run `ewancli worker` or `ewancli serve` to consume it."
        )
