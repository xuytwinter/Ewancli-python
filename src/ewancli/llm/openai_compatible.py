...
from ewancli.llm.pricing import CostBreakdown, ModelPriceProfile, calculate_cost
        ...
        *,
        ...
        if self.price_profile is None:
            raise ValueError(
                ...
                "Set llm.prices in Ewancli config."
            )
        ...
        *,
        ...
        if not self.api_key:
            yield {
                "type": "error",
                "error": RuntimeError(
                    "EWANCLI_API_KEY is not configured. Set it in env, ~/.ewancli/config.json, "
                    "or project .ewancli/config.json."
                ),
            }
            return
        ...
        headers = {
            ...
            "user-agent": "Ewancli-Python/0.1.0",
        }
