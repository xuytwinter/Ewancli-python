...
from ewancli.agent.orchestrator import AgentOrchestrator
from ewancli.agent.plan_execute import PlanExecuteAgent
from ewancli.config import EwanCliConfig
from ewancli.context import ContextBudget, ContextWindowManager
from ewancli.image import parse_image_references
from ewancli.llm.base import LlmClient
from ewancli.prompt import PromptAssembler
from ewancli.skill import SkillContextBuffer, SkillRegistry
from ewancli.snapshot import SnapshotService
from ewancli.tools.base import ToolContext
from ewancli.tools.executor import ToolExecutor
from ewancli.tools.registry import ToolRegistry
from ewancli.types import Message, QueryResult, Usage
    ...
    def _validate_config(self) -> None:
        ...
        if not self.config.llm.api_key:
            raise ValueError(
                ...
                "Set EWANCLI_API_KEY (or DEEPSEEK_API_KEY / GLM_API_KEY / etc.) "
                "in the environment or in ~/.ewancli/config.json."
            )
        if not self.llm_client.max_context_window:
            raise ValueError(
                "LLM context window is not configured. "
                "Set a EWANCLI_CONTEXT_WINDOW environment variable or configure "
                "it in ~/.ewancli/config.json."
            )
...
def _prepend_skill_candidates(user_message: str, cwd: str, config: EwanCliConfig) -> str:
