...
from ewancli.runtime.tasks import TASK_MODES, DurableTaskManager, TaskRecord
from ewancli.types import Message
...
class RuntimeApiServer:
    def __init__(
        ...
        *,
        ...
        self.port = port
        self.db_path = Path.home() / ".ewancli" / "runtime" / "runtime.db"
        self.db_path.parent.mkdir(parents=True, exist_ok=True)
        self.task_manager = DurableTaskManager(
            Path.home() / ".ewancli" / "tasks" / "tasks.db", scope=self.cwd
        )
    ...
    def serve_forever(self) -> None:
        ...
        print(f"Ewancli Runtime API listening on http://127.0.0.1:{self.port}", flush=True)
        try:
    ...
    def work_forever(self) -> None:
        ...
        self._start_workers()
        print(f"Ewancli task worker running with {self.workers} workers", flush=True)
        try:
    ...
    def _worker_loop(self, worker_id: str) -> None:
        while not self._stop.is_set():
            ...
            heartbeat = threading.Thread(
                ...
                name=f"ewancli-heartbeat-{task.id}",
                daemon=True,
            )
            heartbeat.start()
            try:
    ...
    def _ensure_llm_key(self) -> None:
        if not self.config.llm.api_key:
            raise ValueError(
                "EWANCLI_API_KEY is not configured. Runtime turns/tasks need a working LLM key."
            )
...
def runtime_api_key(explicit: str | None = None) -> str:
    key = explicit or os.environ.get("EWANCLI_RUNTIME_API_KEY")
    if not key:
        raise ValueError("EWANCLI_RUNTIME_API_KEY is required for Runtime API")
    return key
