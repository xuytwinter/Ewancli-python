...
class MemoryConfig:
    ...
    long_term_db_path: str = "~/.ewancli/memory.db"
...
class PolicyConfig:
    ...
    audit_log_path: str = "~/.ewancli/audit.jsonl"
...
def load_config(
    ...
    data = _config_to_dict(EwanCliConfig())

    user_config = _read_json(_home() / ".ewancli" / "config.json")
    if user_config:
        data = _deep_merge(data, user_config)

    root = Path(project_root).resolve() if project_root else None
    if root:
        project_config = _read_json(root / ".ewancli" / "config.json")
        if project_config:
...
def get_config_paths(project_root: str | Path | None = None) -> list[Path]:
    paths = [_home() / ".ewancli" / "config.json"]
    if project_root:
        paths.append(Path(project_root).resolve() / ".ewancli" / "config.json")
    return paths


def config_to_public_dict(config: EwanCliConfig) -> dict[str, Any]:
...
def _apply_env(data: dict[str, Any], env: dict[str, str | None]) -> dict[str, Any]:
    ...
    mappings: list[tuple[str, str, Any]] = [
        ("EWANCLI_API_KEY", "api_key", str),
        ("EWANCLI_PROVIDER", "provider", str),
        ("EWANCLI_MODEL", "model", str),
        ("EWANCLI_BASE_URL", "base_url", str),
        ("EWANCLI_CONTEXT_WINDOW", "context_window", int),
        ("EWANCLI_MAX_TOKENS", "max_tokens", int),
        ("EWANCLI_TEMPERATURE", "temperature", float),
    ]
    ...
    render_mode = env.get("EWANCLI_RENDER_MODE") or env.get("EWANCLI_RENDERER")
    if render_mode in {"plain", "inline"}:
        result["render_mode"] = render_mode

    if env.get("EWANCLI_TUI") == "true":
        result["render_mode"] = "inline"

    for env_key, feature_key in [
        ("EWANCLI_MCP", "mcp"),
        ("EWANCLI_SKILL", "skill"),
        ("EWANCLI_MEMORY", "memory"),
    ]:
        raw = env.get(env_key)
        if raw == "false":
...
def _config_to_dict(config: EwanCliConfig) -> dict[str, Any]:
    return asdict(config)


def _dict_to_config(data: dict[str, Any]) -> EwanCliConfig:
    return EwanCliConfig(
